    ...
    <main class="content-area">
      ...
      <div id="mainView" class="view active">
        ...
        <div class="card">
          <div class="card-title">
            ...
            <span data-en="Community Stats" data-ar="إحصائيات المجتمع">Community Stats</span>
          </div>
          ...
          <div class="safety-hub">
            <div class="weather-alert" id="weatherAlert">
              ...
              <div class="alert-content">
                <span class="alert-title" data-en="Weather Alert" data-ar="تنبيه جوي">Weather Alert</span>
                <span class="alert-message" data-en="Loading weather information..." data-ar="جاري تحميل معلومات الطقس...">Loading weather information...</span>
              </div>
            </div>
      ...
      <div id="reportsView" class="view">
        <div class="card">
          <div class="card-title">
            ...
            <span data-en="My Dashboard" data-ar="لوحة التحكم الخاصة بي">My Dashboard</span>
          </div>
          ...
          <div class="card">
            <div class="card-title">
              ...
              <span data-en="Achievements" data-ar="الإنجازات">Achievements</span>
            </div>
          ...
          <div class="card">
            <div class="card-title">
              <i class="fas fa-chart-line"></i>
              <span data-en="My Impact" data-ar="تأثيري">My Impact</span>
            </div>
      ...
      <div id="settingsView" class="view">
        <div class="card">
          ...
          <div class="settings-section">
            <h3 data-en="Privacy & Policy" data-ar="الخصوصية والسياسة">Privacy & Policy</h3>
            <div class="setting-item">
              <span data-en="Data Collection" data-ar="جمع البيانات">Data Collection</span>
              <button class="btn" data-action="privacy-policy" data-en="View Policy" data-ar="عرض السياسة">View Policy</button>
            </div>
            <div class="setting-item">
              <span data-en="Location Data" data-ar="بيانات الموقع">Location Data</span>
              <span data-en="Used for safety reports only" data-ar="تُستخدم لتقارير السلامة فقط">Used for safety reports only</span>
            </div>
            <div class="setting-item">
              <span data-en="Anonymous Reporting" data-ar="الإبلاغ المجهول">Anonymous Reporting</span>
              <span data-en="Reports are anonymous by default" data-ar="التقارير مجهولة افتراضياً">Reports are anonymous by default</span>
            </div>
          </div>
  ...
  <script>
    ...
    class HyperApp {
      ...
      updateStats() {
        ...
        // Update stats display - ensure it works on both desktop and mobile
        const totalReportsElement = document.getElementById('totalReports');
        const activeUsersElement = document.getElementById('activeUsers');
        ...
        if (totalReportsElement) {
          ...
          totalReportsElement.style.display = 'block';
          totalReportsElement.offsetHeight; // Trigger reflow
          totalReportsElement.style.display = '';
        }
        if (activeUsersElement) {
          activeUsersElement.textContent = activeUsers;
          // Force style recalculation for mobile
          activeUsersElement.style.display = 'block';
          activeUsersElement.offsetHeight; // Trigger reflow
          activeUsersElement.style.display = '';
        }
      ...
      showVibeCategoryReports(vibeType) {
        ...
        const modalContent = `
          <div class="modal-content">
            <div class="modal-header">
              <h2 data-en="${this.capitalizeFirstLetter(vibeType)} Reports" data-ar="تقارير ${this.getVibeArabicName(vibeType)}">
                ${this.capitalizeFirstLetter(vibeType)} Reports
              </h2>
              <span class="close" data-dismiss="vibeCategoryModal">&times;</span>
            </div>
            <div class="modal-body">
              <p data-en="Showing ${filteredReports.length} ${vibeType} report${filteredReports.length !== 1 ? 's' : ''}" data-ar="عرض ${filteredReports.length} تقرير ${this.getVibeArabicName(vibeType)}">
              ...
              <div class="reports-container">
                ...
                `).join('') : `
                  <div class="no-data" data-en="No ${vibeType} reports found" data-ar="لم يتم العثور على تقارير ${this.getVibeArabicName(vibeType)}">
      ...
      updateWeatherAlert(weatherData) {
        ...
        if (!weatherData) {
          // Fallback message
          if (alertTitle) alertTitle.textContent = this.currentLanguage === 'en' ? 'Weather Alert' : 'تنبيه جوي';
          if (alertMessage) alertMessage.textContent = this.currentLanguage === 'en'
            ? 'Stay safe during weather changes'
            : 'ابق آمناً أثناء تغيرات الطقس';
        ...
        if (isSevere) {
          if (alertTitle) alertTitle.textContent = this.currentLanguage === 'en' ? 'Weather Warning' : 'تحذير جوي';
          if (alertMessage) alertMessage.textContent = this.currentLanguage === 'en'
            ? `${temp}°C - ${description}. Take precautions!`
            : `${temp}°C - ${description}. خذ احتياطاتك!`;
          weatherAlert.style.borderColor = 'rgba(255, 193, 7, 0.5)';
          weatherAlert.style.background = 'rgba(255, 193, 7, 0.1)';
        } else {
          if (alertTitle) alertTitle.textContent = this.currentLanguage === 'en' ? 'Weather Update' : 'تحديث جوي';
          if (alertMessage) alertMessage.textContent = this.currentLanguage === 'en'
            ? `${temp}°C - ${description}. Enjoy your day!`
            : `${temp}°C - ${description}. استمتع بيومك!`;
