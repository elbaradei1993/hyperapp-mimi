...
import { ruleBasedSafetyAssistant } from '../services/ruleBasedSafetyAssistant';
...
const VoiceChatModal: React.FC<VoiceChatModalProps> = ({
  ...
  const processTranscript = async (transcriptText: string) => {
    try {
      ...
      // Get safety assistant response
      const safetyResponse = await ruleBasedSafetyAssistant.processQuery(
        transcriptText,
        userLocation,
        nearbyReports
      );

      setResponse(safetyResponse);
      setRecordingState('speaking');

      // Speak response if TTS is enabled
      if (isTTSEnabled) {
        speakText(safetyResponse);
      } else {
        setRecordingState('idle');
      }
    } catch (error) {
      console.error('Error getting safetyfety response:', error);
